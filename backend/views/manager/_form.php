...
?>
    ...
    <div class="content">
        ...
        <h4 class="mb-4"><?= $model->isNewRecord ? 'Yangi meneger qo\'shish' : 'Menegerni tahrirlash' ?></h4>
            ...
            <div class="col-lg-10 mx-auto">
                ...
                <div class="card mb-0">
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            <label class="form-label">To'liq ismi<span class="text-danger ms-1">*</span></label>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label">Lavozim (O'zbekcha)<span class="text-danger ms-1">*</span></label>
                                    <?= $form->field($model, 'position_uz')->textInput(['maxlength' => true, 'class' => 'form-control'])->label(false) ?>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label">Lavozim (Ruscha)<span class="text-danger ms-1">*</span></label>
                                    <?= $form->field($model, 'position_ru')->textInput(['maxlength' => true, 'class' => 'form-control'])->label(false) ?>
                                </div>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Biografiya (O'zbekcha)</label>
                            <?= $form->field($model, 'bio_uz')->textarea(['rows' => 5, 'class' => 'form-control'])->label(false) ?>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Biografiya (Ruscha)</label>
                            <?= $form->field($model, 'bio_ru')->textarea(['rows' => 5, 'class' => 'form-control'])->label(false) ?>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <!-- Preview yangi rasm -->
                            <div id="new-photo-preview" class="mt-3" style="display: none;">
                                <label class="form-label">Yangi tanlangan rasm:</label>
                                <div id="photo-preview-container"></div>
                            </div>

                            <?php if (!$model->isNewRecord && $model->photo): ?>
                                <div class="existing-photo mt-3">
                                    <label class="form-label">Mavjud rasm:</label>
                                    <div class="border rounded p-3 d-inline-block position-relative">
                                        <img src="/<?= $model->photo ?>" class="img-fluid rounded" style="max-height: 200px;">
                                        <button type="button" class="btn btn-danger btn-sm mt-2 w-100 delete-photo" data-id="<?= $model->id ?>">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="me-3">
                                            <input type="radio" name="Manager[status]" value="1" id="status-active" <?= ($model->isNewRecord || $model->status == 1) ? 'checked' : '' ?>>
                                            <label for="status-active" class="ms-1">Faol</label>
                                        </div>
                                        <div>
                                            <input type="radio" name="Manager[status]" value="0" id="status-inactive" <?= (!$model->isNewRecord && $model->status == 0) ? 'checked' : '' ?>>
...
$this->registerJs("
    // Rasm preview
    $('#photo-upload-input').on('change', function(e) {
        ...
        if (file && file.type.match('image.*')) {
            ...
            reader.onload = function(e) {
                var html = '<div class=\"border rounded p-3 d-inline-block position-relative\">' +
                    ...
                    '</div>';
                $('#photo-preview-container').html(html);
            };
    ...
    $('.delete-photo').click(function() {
        ...
        $.ajax({
            url: '/manager/delete-photo?id=' + id,
            type: 'POST',
            data: {
            ...
            success: function(response) {
                if (response.success) {
                    $('.existing-photo').fadeOut(300, function() {
                        $(this).remove();
                    });
